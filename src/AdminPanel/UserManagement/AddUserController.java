...
public class AddUserController implements Initializable {
   ...
   private String path="";
   public static long idUserr;
   public static boolean backClickedd=false;
...
private boolean verifDate(LocalDate d) 
   {boolean test=true; 
        if (d==null) //si l'utilisateur n'ai seléctionnée aucune date
            ...
            BirthDate.setStyle("-fx-text-inner-color: rgb(138,198,209) ;");     
            test=false;}        
        else {//Sinon
    ...
    @FXML
     ...
     private void backMouseClicked() {
        User_ManagementController.nbrr=0;
        AddUserController.backClickedd=true;
    ...
    private void imageChooser_MouseClicked(MouseEvent event) {
       ...
       FileChooser fileChooser = new FileChooser();
        ...
        path =selectedFile.toURI().toString();//pour utiliser path lors de l'ajout de cet utilisateur et le stocker dans la BD
        Image image;
    ...
    private void saveAddButton_MouseClicked(MouseEvent event) {
        ...
        firstName.setText("");
        firstName.setStyle("-fx-text-inner-color: rgb(138,198,209) ;");     
        }
        ...
        lastName.setText("");
        lastName.setStyle("-fx-text-inner-color: rgb(138,198,209) ;");     
        }
       ...
       if (profession.getValue()==null)
        ...
        address.setText("");
        address.setStyle("-fx-text-inner-color: rgb(138,198,209) ;");     
        }
       ...
       if (!Register_SignInForms.RegisterFormController.verifEmail(email.getText()))
        ...
        email.setText("");
        email.setStyle("-fx-text-inner-color: rgb(138,198,209) ;");     
        }
       ...
       if ((!Register_SignInForms.RegisterFormController.verifLogin(login.getText()))||login_existance.getText().equals("USED")) //soit le login est mal saisi (ne satisfait pas les conditions) soit il est déjà utilisé que ce soit par un admin ou un utilisateur normal 
        ...
        login_existance_png.setImage(null);
        login.setStyle("-fx-text-inner-color: rgb(138,198,209) ;");     
        }
        else {
        ErrLogin.setText("");
        login.setStyle("-fx-text-inner-color: black ;");     
         ...
         {i=1;
        ErrPassword.setText("Please enter a valid\npassword !");
        password.setText("");
        security.setText("");
        password.setStyle("-fx-text-inner-color: rgb(138,198,209) ;"); 
        passwordtxt.setText("");
        passwordtxt.setStyle("-fx-text-inner-color: rgb(138,198,209) ;");     
         ...
         {Register_SignInForms.UserRegistration regU=new Register_SignInForms.UserRegistration();
        regU.setFirst_name(firstName.getText());
        regU.setLast_name(lastName.getText());

        String birth_date = BirthDate.getValue().format(DateTimeFormatter.ofPattern("dd/MM/yyyy"));
        regU.setDate_birth(birth_date);

            if (male.isSelected()==false )
                regU.setGender(female.getText());
            else
                regU.setGender(male.getText());

        regU.setProfession(profession.getValue());
        regU.setAddress(address.getText());
        regU.setEmail(email.getText());
        regU.setLogin(login.getText());
        regU.setPasswrd(password.getText());
        
             try{//insertion du nouvel utilisateur dans la BD puis rafraîchissement de la tableview 
        Register_SignInForms.ConnexionDB conn= new Register_SignInForms.ConnexionDB();
        String row="INSERT INTO USERS (ID_USER,FIRST_NAME,LAST_NAME,BIRTH_DATE,GENDER,PROFESSION,ADDRESS,EMAIL,LOGIN,PASSWORD,PROFILE_PICTURE) VALUES ("+idUserr+",'"+regU.getFirst_name()+"','"+regU.getLast_name()+"',TO_DATE('"+regU.getDate_birth()+"'),'"+regU.getGender()+"','"+regU.getProfession()+"','"+regU.getAddress()+"','"+regU.getEmail()+"','"+regU.getLogin()+"','"+regU.getPasswrd()+"','"+path+"')";
        ...
        if (rss==0)
               ...
               alert.showAndWait();
               AddUserController.backClickedd=false;
               
               User_ManagementController.nbrr=0;//lorsque tout est bon et l'inscription est effectuée avec succès la fenêtre add user va être fermée et le compteur nbr (qui nous renseigne sur le nombre de fois la fenêtre add user est ouverte) sera initialisé à 0
