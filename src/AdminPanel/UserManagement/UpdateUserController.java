   ...
   {boolean test=true; 
    ...
    @FXML
     ...
     private void backMouseClicked() {
        User_ManagementController.nbrr2=0;
    ...
    private void imageChooser_MouseClicked(MouseEvent event) {
       ...
       FileChooser fileChooser = new FileChooser();
        ...
        path =selectedFile.toURI().toString();//pour utiliser path lors de la mise à jour de cet utilisateur et le stocker dans la BD
    ...
    private void saveUpdateButton_MouseClicked(MouseEvent event) {
       ...
       if ((!Register_SignInForms.RegisterFormController.verifLogin(login.getText()))||login_existance.getText().equals("USED")) //soit le login est mal saisi (ne satisfait pas les conditions) soit il est déjà utilisé que ce soit par un admin ou un utilisateur normal
        ...
        passwordtxt.setStyle("-fx-text-inner-color: rgb(130,145,255) ;");     
         ...
         {Register_SignInForms.UserRegistration regA=new Register_SignInForms.UserRegistration();
         regA.setIDUser(Long.parseLong(id_user_txt.getText()));
        ...
        regA.setPasswrd(password.getText());
             ...
             try{//update de l'utilisateur dans la BD puis rafraîchissement de la tableview 
        Register_SignInForms.ConnexionDB conn= new Register_SignInForms.ConnexionDB();
        String row="UPDATE USERS SET FIRST_NAME='"+regA.getFirst_name()+"',"
                ...
                + "PROFILE_PICTURE='"+path+"' where ID_USER="+regA.getIDUser();
        ...
        if (rss==0)
               ...
               User_ManagementController.nbrr2=0;//lorsque tout est bon et la modification est effectuée avec succès la fenêtre update user va être fermée et le compteur nbr (qui nous renseigne sur le nombre de fois la fenêtre update user est ouverte) sera initialisé à 0
