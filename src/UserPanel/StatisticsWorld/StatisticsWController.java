 ...
 */
package UserPanel.StatisticsWorld;
 ...
 * @author selmi
 */
public class StatisticsWController implements Initializable {
   ...
   (observable, oldToggle, newToggle) -> {
    ...
    private void Distribution(){
        ...
        try {
                ConnexionDB cnx=new ConnexionDB() ;
                String sql ;
                    sql="select country,TotalCases from coronaWorld " 
    ...
    private void DeathsVSRecovered() {
        ...
        series1.setName("2020");  
         try {
                ConnexionDB cnx=new ConnexionDB() ;
                String sql ;
                    sql="select sum(TotalDeaths),sum(TotalRecovered) from coronaWorld "
                            + "where to_char(dateupdate,'dd-mm-yyyy') = to_char(sysdate,'dd-mm-yyyy') " ;
                    ResultSet rs=null;
                rs=cnx.getStmt().executeQuery(sql) ;//execute select query
                if(rs.next()){
