...
public class RegisterFormController implements Initializable {
    ...
    public static boolean verifAlpha(String ch) //méthode qui assure que toute la chaîne ne contient que des lettres ou espace
    { boolean test=true;
    ...
    {
        if ((!Character.isAlphabetic(ch.charAt(i)))&&(!(ch.charAt(i)==' ')))
        {test=false;
        break;            
        }
    }
   ...
   public static boolean verifEmail(String ch) //fonction qui vérifie que l'email saisi est correct (il contient un @ suivi par un point)
    ...
    return test;
     ...
     @FXML
     private void registerMouseClicked() throws IOException {
       ...
       if ((!verifLogin(login.getText()))||login_existance.getText().equals("USED")) //soit le login est mal saisi (ne satisfait pas les conditions) soit il est déjà utilisé que ce soit par un admin ou un utilisateur normal
         {i=1;
         ErrLogin.setText("Please enter a valid \nLOGIN ! Length > 6 and NO spaces or :");
        login.setText("");
        ...
        {i=1;
        ErrTerms.setText("The Terms of Service must be accepted !");            
        }
        else {ErrTerms.setText("");
        }
    ...
    try{
        ...
        String sql="INSERT INTO USERS (ID_USER,FIRST_NAME,LAST_NAME,BIRTH_DATE,GENDER,PROFESSION,ADDRESS,EMAIL,LOGIN,PASSWORD,PROFILE_PICTURE)"+"VALUES("+"Users_auto_increment.nextval"+",'"+reg.getFirst_name()+"','"+reg.getLast_name()+"', TO_DATE('"+reg.getDate_birth()+"'),'"+reg.getGender()+"','"+reg.getProfession()+"','"+reg.getAddress()+"','"+reg.getEmail()+"','"+reg.getLogin()+"','"+reg.getPasswrd()+"','"+path+"')";
